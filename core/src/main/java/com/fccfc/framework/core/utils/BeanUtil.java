...
public final class BeanUtil {
	...
	public static String[] getMethodParamNames(Class<?> clazz, String method,
			Class<?>... paramTypes) throws NotFoundException, UtilException {
	...
	public static String[] getMethodParamNames(Method method)
		...
		return getMethodParamNames(method.getDeclaringClass(),
	...
	protected static String[] getMethodParamNames(CtMethod cm)
		...
		if (codeAttribute == null) {
			throw new UtilException("class:{" + cc.getName()
					+ "} 不包含局部变量表信息，请使用编译器选项 javac -g:'{'vars'}'来编译源文件。");
		}
	...
	public static Set<Class<?>> getClasses(String pack) throws UtilException {
		...
		if (pack.lastIndexOf(SUB_PACKAGE_SCREEN_SUFFIX) != -1) {
			...
			if (packArr.length > 1) {
				...
				for (int i = 0; i < packArr.length; i++) {
					packArr[i] = packArr[i].replace(
							SUB_PACKAGE_SCREEN_SUFFIX.substring(1), "");
				}
			} else {
		...
		try {
			...
			throw new UtilException("扫描Jar包文件失败");
		}
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if ((i >= 0) && Character.isUpperCase(c)) {
				if (!upperCase || !nextUpperCase) {
					if (i > 0)
						sb.append(DaoConstants.UNDERLINE);
				}
				upperCase = true;
			} else {
				upperCase = false;
